...
/*
    ...
    Return value of true indicates simulation should end.
*/
bool RVSimulator::InstructionLoop()
{
    uint32_t Instruction = 0; //To hold instruction loaded from memory.
    ...
    switch(Opcode)
    {
        case OP_IMMEDIATE: //I-type instructions.
            ...
            break;
        case OP_JALR:
            ReturnValue = I_Instructions(Instruction);
            break;
...
void RVSimulator::Simulate(const char *MemoryFile, bool GUI)
{
    ...
    if(!ReadFile(MemoryFile)) //If we failed to read the file.
    {
        if(!GUI) //If in GUI, user can still select file.
        {
            return;
        }
    }
...
bool RVSimulator::ReadFile(const char *MemoryFile)
{
